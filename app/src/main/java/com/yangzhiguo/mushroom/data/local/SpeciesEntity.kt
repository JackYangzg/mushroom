...
/**
 * Core mushroom species row in `mushroom_species` (schema v11).
 *
 * v10 重设计:
 * - `id` (Long, autoGenerate) 是 Room 内部主键,**不**对外暴露、不绑定业务。
 * - `mushroom_id` (Int) 是来自 iflora API 的业务 id,UI / 跨表关联 / 缓存命名
 *   全部走 mushroom_id。
 * - 唯一键是 `(scraw_source, source_url)`:同一 source 对同一 source_url 的重抓
 *   走 `@Upsert` 覆盖,不会产生重复行。同一 mushroom_id 在多个 source 出现时
 *   会有多行(mushroom_id 不唯一)。
 *
 * 子表只保留 `mushroom_image`(按 mushroom_id 关联)。原 specimen / DNA / distribution
 * 三张子表已删除。
 *
...
@Entity(
    ...
    indices = [
        // 唯一键 (scraw_source, source_url) — @Upsert 的去重依据
        Index(
            value = ["scraw_source", "source_url"],
            unique = true,
            name = "idx_species_scraw_source_url",
        ),
        // 业务 id 索引 — mushroom_id 不唯一(同一 id 跨 source 多行),但 DAO 的
        // findByMushroomId / observeAll ORDER BY 都需要
        Index(value = ["mushroom_id"], name = "idx_species_mushroom_id"),
