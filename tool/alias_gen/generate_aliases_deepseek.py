#!/usr/bin/env python3
"""Batch-generate mushroom aliases via DeepSeek and write to alias_mushroom.db.
...
API    : DeepSeek Chat Completions (OpenAI-compatible)  — model deepseek-v4-flash

Usage:
  python3 generate_aliases_deepseek.py [--batch-size 30] [--max-retries 5] [--limit 0]
"""
...
# DeepSeek config
# ---------------------------------------------------------------------------
API_BASE: str = os.environ.get("DEEPSEEK_BASE_URL", "https://api.deepseek.com").rstrip("/")
API_KEY: str = os.environ.get(
    "DEEPSEEK_API_KEY",
    "sk-f14e194c965a4500bdfd761c9220b1d3",
)
...
def export_species_jsonl(species: list[dict]) -> None:
    """Write a JSON-lines file so retry scripts can look up common_name by id."""
...
def call_llm(batch_items: list[dict], retries: int) -> dict:
    """Call the DeepSeek Chat Completions API and return parsed JSON dict.

    DeepSeek's endpoint is OpenAI-compatible (`/chat/completions`). We request
    JSON-mode via `response_format={"type":"json_object"}` so the model is
    forced to emit a single JSON object containing the `items` array.
    """
    ...
    body: dict[str, Any] = {
        "model": MODEL,
        "messages": [
            {"role": "system", "content": "你是真菌名称标准化助手。必须输出严格 JSON。"},
...
def insert_aliases(conn: sqlite3.Connection, results: list[dict]) -> int:
    ...
    rows: list[tuple] = []
    for item in results:
        sci: str = item.get("scientific_name", "")
        com: str = item.get("common_name", "")
...
def main() -> None:
    ...
    if not API_KEY:
        log(c("red", "ERROR: DEEPSEEK_API_KEY env var is empty. Aborting."))
        sys.exit(1)
    ...
    log_step(f"Batch size: {batch_size}  ->  {len(batches)} batches  (通常需要 10s ~ 30s 时间)")
    log(c("dim", "─" * 60))
    ...
    if failed_items:
        ...
        log(c("yellow", f"  → wrote failed_ids.txt ({len(failed_items)} ids). Re-run with --resume later."))
    else:
